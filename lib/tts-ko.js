...
const CONS = {
  "ㄱ": ["기역", "가"], "ㄴ": ["니은", "나"], "ㄷ": ["디귿", "다"], "ㄹ": ["리을", "라"],
  "ㅁ": ["미음", "마"], "ㅂ": ["비읍", "바"], "ㅅ": ["시옷", "사"], "ㅇ": ["이응", "아"],
  "ㅈ": ["지읒", "자"], "ㅊ": ["치읓", "차"], "ㅋ": ["키읔", "카"], "ㅌ": ["티읕", "타"],
  "ㅍ": ["피읖", "파"], "ㅎ": ["히읗", "하"],
  "ㄲ": ["쌍기역", "까"], "ㄸ": ["쌍디귿", "따"], "ㅃ": ["쌍비읍", "빠"],
  "ㅆ": ["쌍시옷", "싸"], "ㅉ": ["쌍지읒", "짜"],
};
 ...
 * @param {{demo?:boolean}} opts demo=true → у согласной добавить демо-слог
...
export function toSpeakable(text, opts = {}) {
  ...
  if (CONS[t]) {
    ...
    return { speak: opts.demo ? `${name}. ${demo}.` : name, kind: "consonant" };
  }
 ...
 */
const ENGINES = {
  // Inworld TTS-1.5 Max — нативные корейские голоса, $0.01/1K симв., около 2 с из РФ.
  // Минус: НЕТ управления скоростью, замедление делаем только пре-рендером (см. cache).
  "inworld-minji": {
