...
// не было: память о фанате (`idol_memory`), код-гейт против учебной повестки (`agendaPressure`)
...
const MEMORY_LIMIT = 25; // сколько фактов о фанате держим (≈300 токенов)
const MEMORY_EVERY = 5; // раз в сколько сообщений фаната обновляем память
...
function memoryBlock(rows) {
  ...
  return [
    "",
    "ЧТО ТЫ ЗНАЕШЬ ПРО НЕГО (твоя память, накопленная из ваших разговоров):",
    lines,
...
const EXTRACT_SYSTEM = [
  "Ты — экстрактор фактов. На вход даётся кусок переписки между фанатом (Фанат) и айдолом (Айдол).",
  "Верни СТРОГО JSON-объект вида {\"ключ\":\"значение\"} с фактами О ФАНАТЕ, которые он сам сообщил и которые стоит помнить другу.",
  "Ключи — короткие латиницей snake_case: name, city, country, timezone, age_group, job_or_study, bias, favorite_group, favorite_song, mbti, pet, food, hobby, mood_recent, plan, learned_word, open_thread.",
  "Значения — короткая фраза до 120 символов, на языке фаната.",
  "Бери только явно сказанное фанатом. Не додумывай, не переноси факты об айдоле, не выдумывай.",
  "Если ничего нового нет — верни {}. Никакого текста вокруг JSON.",
].join("\n");
...
export async function extractMemory(db, idol, chrono) {
  const tail = (chrono || []).slice(-12);
  ...
  for (const [k, v] of Object.entries(obj)) {
    ...
    const key = String(k).trim().toLowerCase().replace(/[^a-z0-9_]+/g, "_").slice(0, 32);
    const value = typeof v === "string" ? v.trim() : Array.isArray(v) ? v.join(", ") : String(v ?? "").trim();
    if (!key || !value || value.length > 160) continue;
    rows.push({ idol_id: idol.id, key, value, updated_at: new Date().toISOString() });
  }
  if (!rows.length) return;

  try {
    await db.from("idol_memory").upsert(rows, { onConflict: "idol_id,key" });
    // Обрезаем хвост: держим не больше MEMORY_LIMIT самых свежих фактов на айдола.
    const { data: all } = await db
      .from("idol_memory")
      .select("key,updated_at")
      .eq("idol_id", idol.id)
      .order("updated_at", { ascending: false });
    if (all && all.length > MEMORY_LIMIT) {
      const stale = all.slice(MEMORY_LIMIT).map((r) => r.key);
      await db.from("idol_memory").delete().eq("idol_id", idol.id).in("key", stale);
    }
  } catch {
...
export async function generateReply({ db, idol, text, lang = "en", channel = "web" }) {
  ...
  // Память обновляем раз в MEMORY_EVERY сообщений и СТРОГО параллельно ответу:
  // общая задержка = max(ответ, экстрактор), а экстрактор ограничен EXTRACT_TIMEOUT_MS.
  const wantMemory = lim.used % MEMORY_EVERY === 0;
  const memoryTask = wantMemory ? extractMemory(db, idol, chrono).catch(() => {}) : null;
