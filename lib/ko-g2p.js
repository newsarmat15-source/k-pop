...
// Кириллица по системе Концевича. Плоские ㄱㄷㅂㅈ здесь в ЗВОНКОМ (междугласном)
// виде г/д/б/дж — это значение по умолчанию для середины слова; в начале слова они
// оглушаются до к/т/п/ч (см. CYR_CHO_VL и romanizeSyl). Придыхательные несут «х»
// (кх/тх/пх/чх), иначе в начале слова совпали бы с оглушённым плоским (자 «ча» vs 차 «чха»).
const CYR_CHO = { "ㄱ":"г","ㄲ":"кк","ㄴ":"н","ㄷ":"д","ㄸ":"тт","ㄹ":"р","ㅁ":"м","ㅂ":"б","ㅃ":"пп",
                  "ㅅ":"с","ㅆ":"сс","ㅇ":"","ㅈ":"дж","ㅉ":"чч","ㅊ":"чх","ㅋ":"кх","ㅌ":"тх","ㅍ":"пх","ㅎ":"х" };
// Оглушённые начала слова. Кореец в начале слова/в изоляции произносит плоский стоп
// глухо: 가 = «ка», 다 = «та», 바 = «па», 자 = «ча». Между гласными и после сонорных
// (ㄴㅁㅇㄹ) он звончает — тогда действует CYR_CHO. Латиница (RR: g/d/b/j) это не различает.
const CYR_CHO_VL = { "ㄱ":"к","ㄷ":"т","ㅂ":"п","ㅈ":"ч" };
// Коды, после которых плоский стоп остаётся звонким: открытый слог («») и сонорные.
const VOICED_CODA = new Set(["","ㄴ","ㅁ","ㅇ","ㄹ"]);
...
function romanizeSyl(s, cyr, prev) {
  ...
  // Оглушение плоского стопа в начале слова: 가방 → «ка-бан», 지금 → «чи-гым».
  // Звонкий вариант (г/д/б/дж) остаётся, только если перед стопом звонкий контекст —
  // гласная предыдущего слога или сонорный код (야구 → «я-гу», 여자 → «ё-джа»).
  if (cyr && CYR_CHO_VL[s.c]) {
    const voicedCtx = prev && prev.raw === undefined && VOICED_CODA.has(prev.f || "");
    if (!voicedCtx) onset = CYR_CHO_VL[s.c];
  }
