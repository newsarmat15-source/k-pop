...
function karaBuild(song){
  /* СВОИ ТАЙМИНГИ СЛОВ ИЛИ РАСЧЁТ — РЕШАЕТСЯ ПО КАЖДОЙ СТРОКЕ ОТДЕЛЬНО.
     Было: один флаг на всю песню, поднятый по первому слову первой строки. У 봄날
     размеченный вручную только первый куплет, у второго и третьего таймингов слов нет —
     и весь второй-третий куплет получал t0=0. Тайминги схлопывались в ноль, «конец
     песни» оказывался на 0,6 секунды, и караоке гасло сразу после старта, открывая
     набор слов. Ровно то, что Сармат видел как «нажимаю плей, через 3 секунды караоке
     пропадает». Теперь строка со своими таймингами берёт их, строка без них считается
     по длине слогов внутри своего окна — соседи друг друга больше не ломают. */
  const verses=song.verses.map(v=>{
    const lines=v.lines.map((ln,i)=>{
      let words;
      const exact=(ln.w||[]).length>0&&ln.w.every(x=>x&&x.t!=null);
      if(exact){
        words=ln.w.map(x=>({...x,t0:x.t,te:(x.te!=null?x.te:null)}));
      }else{
  ...
  // Конец куплета = начало следующего. У последнего берём то, что дальше: конец
  // последнего слова или объявленный конец куплета в данных. Максимум, а не выбор по
  // флагу: раньше при «своих таймингах» объявленный конец игнорировался, и песня
  // считалась законченной раньше, чем допевалась.
  for(let i=0;i<verses.length;i++){
    if(i+1<verses.length){verses[i].end=verses[i+1].start;continue;}
    const lastW=allW.length?allW[allW.length-1].t1:0;
    verses[i].end=Math.max(lastW,(song.verses[i]&&song.verses[i].end)||0,verses[i].start+4);
  }
...
function openSong(id){
  ...
  // СВЕЖИЙ КАТАЛОГ. loadCatalog() зовётся только при открытии списка, а песню можно
  // открыть напрямую (восстановление вкладки при перезаходе, переход из тетради). Тогда
  // window._catalog пуст, и allSongs() подставляет КОПИЮ из localStorage (so_usersongs) —
  // старую: старое видео, старый разбор, старая транскрипция, без перевода английских
  // строк. Отсюда «вышел-зашёл, ничего не поменялось». Если свежей версии этой песни в
  // каталоге ещё нет — тянем каталог с сервера и открываем уже свежую копию.
  if(id!==LP_SONG && !(window._catalog&&window._catalog.some(s=>s&&s.id===id))){
    fetch('/api/song?action=list').then(r=>r.json()).then(d=>{if(d&&d.songs)window._catalog=d.songs;}).catch(()=>{}).then(()=>_openSongNow(id));
    return;
  }
  _openSongNow(id);
}
function _openSongNow(id){
