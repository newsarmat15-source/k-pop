...
//
// ЕДИНСТВЕННЫЙ ИСТОЧНИК ПРАВДЫ. Раньше файл лежал в lib/ и был доступен только
// серверу (api/song.js), а браузер романизировал слова СВОЕЙ таблицей (wbRomanize
// в app.js) — послоговое чтение без ассимиляции. Из-за этого одно и то же слово
// в разборе песни и в тетради читалось по-разному. Теперь движок один: файл лежит
// в public/ (значит, отдаётся браузеру статикой), а lib/ko-g2p.js — тонкий
// ре-экспорт для серверных импортов. Второй копии правил нет и быть не должно.
//
// Формат намеренно двойной: обычный <script> в браузере и валидный ES-модуль в
// Node (модуль без export'ов — это законный модуль). Наружу отдаётся один объект
// globalThis.KoG2P = { pronounce, transcribe, transcribeLatin, transcribeCyrillic }.
//
...
(function () {
  ...
  function transcribe(word, cyr = false) {
  ...
  const transcribeLatin = (w) => transcribe(w, false);
  const transcribeCyrillic = (w) => transcribe(w, true);
