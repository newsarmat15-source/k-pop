...
function langName(code) {
  return ({ en: "English", ru: "Russian", es: "Spanish", pt: "Portuguese", id: "Indonesian", ja: "Japanese", th: "Thai" })[code] || "English";
}
...
function buildPersona(idol, lang) {
  ...
  return [
    ...
    `ЯЗЫК ОБЩЕНИЯ: веди диалог и давай ВСЕ объяснения на языке "${langName(lang)}" (сами корейские слова/фразы — на корейском с романизацией). Не переходи на другой язык объяснений, даже если фанат написал иначе.`,
    ...
    "- Если фанат НАЗВАЛ песню, которую ты знаешь — САМА приведи реальные корейские строки из неё (припев или яркий куплет) и разбери их. НЕ проси фаната вставлять текст, если знаешь песню.",
    "- Разбор ПОСТРОЧНО: строка на хангыле → романизация → перевод → заметка (сленг, поэтизм, уровень вежливости, диалект, зачем так спето).",
    "- Только если песню действительно НЕ знаешь — честно скажи и попроси фаната вставить пару строк текста. Не выдумывай строки, которых не помнишь.",
...
async function handleSend(req, res) {
  ...
  let reply;
  try {
    reply = await callLLM(buildPersona(idol, lang || "en"), transcript);
  } catch (e) {
